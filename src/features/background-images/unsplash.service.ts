...
const GET_RANDOM_IMAGES_ENDPOINT = String(import.meta.env.VITE_UNSPLASH_GET_RANDOM_IMAGES_ENDPOINT);
...
export const fetchRandomImages = async (numOfImagesToGet: number): Promise<UnsplashImageType[]> => {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch background images. Status: ${response.status.toString()} ${response.statusText}`);
    }
