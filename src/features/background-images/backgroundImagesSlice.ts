    ...
    if (currentDisplayImageIndex < images.length - 1) {
      ...
      void dispatch(
        fetchRandomBackgroundImages({
          fetchReason: BACKGROUND_IMAGE_FETCH_REASONS.FORWARD,
        }),
      );
    }
  };
...
export const navigateBackward =
  ...
  (dispatch, getState) => {
    ...
    if (0 < currentDisplayImageIndex) {
      ...
      void dispatch(
        fetchRandomBackgroundImages({
          fetchReason: BACKGROUND_IMAGE_FETCH_REASONS.BACKWARD,
        }),
      );
    }
  };
...
const backgroundImagesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchRandomBackgroundImages.fulfilled, (state, action) => {
      switch (action.payload.fetchReason) {
        case BACKGROUND_IMAGE_FETCH_REASONS.INIT:
          ...
          state.currentDisplayImageIndex = Math.max(
            0,
            Math.floor(action.payload.newImages.length / 2) - 1,
          );
          break;
        ...
        case BACKGROUND_IMAGE_FETCH_REASONS.BACKWARD:
          ...
          state.currentDisplayImageIndex = Math.max(
            0,
            action.payload.newImages.length - 1,
          );
          break;
      }
    ...
    builder.addMatcher(
      isRejected(fetchRandomBackgroundImages),
      (state, action) => {
...
export const selectIsLoading = (state: {
  backgroundImages: BackgroundImagesSliceStateType;
}) => state.backgroundImages.isLoading;

export const { setCurrentDisplayImageIndex } = backgroundImagesSlice.actions;
